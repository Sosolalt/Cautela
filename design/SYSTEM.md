> **⚠ SUPERSEDED — 2026-06-08.** This is no longer the canonical design system. The current system lives in [`design/claude-design-output/`](claude-design-output/README.md), indexed from [`design/SOURCE_OF_TRUTH.md`](SOURCE_OF_TRUTH.md). The Documentary-Brutalism palette (champagne / oxblood / warm-paper on near-black surface) and Instrument Serif / Space Grotesk / Geist Mono type stack replace everything locked here — including the cool-green-tinted neutrals, warm-clay accent (`#B86F3D` is now explicitly **forbidden**), Fraunces / Inter / JetBrains Mono trinity, 12px Card radius, and the brand-vs-interactive split. `design/tokens.ts` has been revalued to absorb the new palette under the same key names; do not re-introduce values from this file. Audit-trail only.
