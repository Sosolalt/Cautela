> **⚠ SUPERSEDED — 2026-06-08.** Canonical brand → [`design/claude-design-output/`](claude-design-output/README.md); index → [`design/SOURCE_OF_TRUTH.md`](SOURCE_OF_TRUTH.md). The stack candidates locked here (Next.js/Framer Motion/GSAP defaults, hero candidate #2 contract-stack) were sized to a now-retired creative direction. Where this file specifies tooling that is still in the project's STACK.md anti-slop allowlist (R3F, Spline, Rive, Lottie remain forbidden), those bans carry forward; everything else is audit-trail only.
