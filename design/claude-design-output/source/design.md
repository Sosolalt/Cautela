...
> Cluster-bootstrap 95% lower bound — contracts as the IID unit. Wilson 95% LB retained as an exploratory per-finding-IID cross-check. Frozen held-out fold. We report the worst case, not the best.
>
> *Note (2026-06-08, Fix-10 propagation):* original line read *"Wilson lower bounds. Frozen held-out fold. Paired-bootstrap CI gates. We report the worst case, not the best."* — reframed to match `ma_gatekeeper/docs/devpost.md` L99–108, where cluster-bootstrap is the headline statistic and Wilson is demoted to an exploratory per-finding-IID cross-check.
